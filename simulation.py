...
class Simulator:
    def __init__(self, dk=15, energy_convolution_sigma=(8 / 2.355), T=5, scaleup_factor=10000, kf=0.4, c=-1000,
        ...
        self.w = w
        if k_step is None:
            k_step = (1 / H_BAR) * math.sqrt(
                2 * ELECTRON_MASS / LIGHT_SPEED / LIGHT_SPEED * 6176.5840329647) * d_theta / (
                             10 ** 10)
        else:
            k_step = k_step
    ...
    def generate_spectra(self, fermi=True, convolute=True, noise=True):
        ...
        if convolute:
            if fermi:
                Z = I(X, Y, self.a, self.c, self.dk, self.T, self.scaleup_factor, self.energy_convolution_sigma,
                      self.temp)
            else:
